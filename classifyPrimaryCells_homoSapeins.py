##########################################################
## ccAF:  classifyPrimaryCells_homoSapiens.py           ##
##  ______     ______     __  __                        ##
## /\  __ \   /\  ___\   /\ \/\ \                       ##
## \ \  __ \  \ \___  \  \ \ \_\ \                      ##
##  \ \_\ \_\  \/\_____\  \ \_____\                     ##
...
# Load up loom file
for set1 in ['Nowakowski_norm', 'HEK293T','GSE103322']: 
    if not exists('results/table1_'+str(set1)+'.csv'): 
        ...
        if not exists('results/CV_classification_report_'+set1+'.csv'):
            ...
            t1.index = ['Counts']
            if set1=='Nowakowski_norm':
                t2 = confusionMatrix(set1_scanpy.obs['WGCNAcluster_restricted'], set1PredLbls)
